...
The package SHALL export TypeScript types `WidgetPayload`, `WidgetKind`, `WidgetHints`, and `WidgetMeta` describing the contract `{ kind, data, hints?, meta? }`.

#### Scenario: Import types from the package entry
- **WHEN** a consumer imports `WidgetPayload` from the package entry
