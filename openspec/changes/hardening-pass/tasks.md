...
- [x] 1.3 **[USER]** Regenerate the GitHub OAuth client secret (GitHub → Settings → Developer settings → OAuth Apps → client `Ov23likZKNlJwQu8BAvo` → Generate a new client secret; keep the OLD one active until 1.7) and store it: `az keyvault secret set --vault-name widgentickv --name widgentic-github-client-secret --value '<new>' -o none` from your own terminal — never paste it into chat
