...
**D4 — Entra External ID with GitHub federated into it.** Both sign-in methods were requested; federating GitHub *into* External ID means the app validates **one** token type from one issuer instead of implementing two OAuth flows and reconciling identities. Free to 50 k MAU. The app maps the token's subject to a principal id on first sign-in and never sees a password.
