...
- [x] 6.8 Hints join the Tree/JSON pattern: `record-editor.ts` (flat name→doc rows, hover-revealed removal, emptied record drops the key) + tabs in General with a plain "Hints" label (type-signature legend gone); JSON panes indent on Tab via attachJsonHighlight (two spaces at caret, synthetic input keeps store+paint in step; Shift+Tab and readonly keep defaults); delta scenarios + tests

## 7. Read-only mode and the app's view/edit flow (user feedback)

- [x] 7.1 Delta specs: widget-designer "Designer programmatic surface" MODIFIED (readOnly option + setReadOnly on both handles, all original scenarios carried); widgentic-app "Designing and publishing are the same act" MODIFIED (select→read-only, Edit→Save/Cancel, New/Save hidden during edit)
- [x] 7.2 Library: `setReadOnly` on the widget designer (inert section bodies in the definition column + preview-data/styles sections; theme selector and preview stay live) and the theme designer (preview-kind row moves beside the preview so it stays live; identity/tokens/custom/io go inert); `readOnly` option on both factories; de-emphasis styling
- [x] 7.3 App (main.ts): per-tab mode state (new/viewing/editing) — row click loads read-only with Edit/Delete; Edit switches the row to Save/Cancel and hides New + Save-to-my-catalog; Save persists and returns to viewing; Cancel restores the stored entry; Delete of the selection returns to a fresh editable designer
