...
- **Model-facing explanation and composition** (added from live findings): `render_widget`/`execute_action` text ends with an `Action notes:` tail explaining the widget's actions, `load`, and any `disabled` reason; bound widgets keep working inside `group` renders (descriptors carry `widget`/`at`, results fold into the right item, group items load sequentially); the designers gained schema-driven mapping completions with a type-mismatch check, attribute-bind dropdowns, an action menu limited to buttons/links, busy states and a top notification banner.
