...
|---|---|---|
| `kind` | yes | The widget identifier: a built-in (`card`, `table`, `tree`, `group`) or a kind registered on the catalog. A `kind` the catalog does not know is a structured error, never a guess. |
