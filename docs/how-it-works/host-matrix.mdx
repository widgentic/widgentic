...
| ChatGPT (web, mobile web, desktop) | Custom connector by URL, so the key travels as `?key=` | Yes: ChatGPT implements the MCP Apps standard (`ui://` resources, `text/html;profile=mcp-app`); widgets render on chatgpt.com, in a mobile browser and in the desktop app | Yes, but it **sends** rather than prefills: desktop shows a send-or-cancel dialog; on the web with the connector set to "allow all" the message went out with no dialog | Yes: a widget-level `load` binding fired on first render, so `serverTools` is present and the frame's `tools/call` is proxied | The connector's permission setting decides whether prompt actions ask before sending |
...
- All four Apps hosts we probed for actions — basic-host, claude.ai, VS Code Copilot Chat and ChatGPT — honour `ui/message`, but only claude.ai advertises it. Three prefill the composer; ChatGPT SENDS the message, behind a send-or-cancel dialog on desktop and automatically on the web when the connector is set to "allow all". Author prompt actions accordingly: on ChatGPT the text is the message, not a suggestion. The frame therefore never gates prompt actions on the advertised flag; a host that answers with a JSON-RPC error gets an inline alert inside the frame while the widget stays intact.
...
|---|---|---|
| ChatGPT Desktop (built-in browser) | Native; the address bar lists "Available site tools" | Verified: the tools are listed and called. Its agent can also operate the page, so asked to "create a widget" it drafts through the tools and then clicks Save itself under the person's session and permission settings — the draft is visible first; say "draft it, I will save" to keep the click |
| Chrome 149+ / Edge 150 | Origin trial (a token on the page enables it for that origin), or `chrome://flags/#enable-webmcp-testing` | Verified on Chrome 151 both ways — with the testing flag, and with an origin-trial token on a plain Chrome with no flag: the tools listed on `document.modelContext`, `readOnlyHint` intact on the read tools, `executeTool` (input as a JSON string) changed the designer visibly |
