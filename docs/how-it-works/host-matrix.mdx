...
| Chrome 149+ / Edge 150 | Origin trial (a token on the page enables it for that origin), or `chrome://flags/#enable-webmcp-testing` | Verified on Chrome 151 with the testing flag: twelve tools listed on `document.modelContext`, `readOnlyHint` intact on the five read tools, a draft-load call changed the designer visibly |
