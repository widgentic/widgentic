...
| ChatGPT (connectors) | Custom connector by URL, so the key travels as `?key=` | Not probed. widgentic serves the MCP Apps `ui://` template, not ChatGPT's Apps SDK `openai/outputTemplate` convention, so expect the text result until probed | Not probed | Not probed | Its desktop app's browser runs the authoring page's WebMCP tools — see below |
...
|---|---|---|
| ChatGPT Desktop (built-in browser) | Native, per OpenAI's WebMCP documentation; the address bar lists "Available site tools" | Documented by OpenAI; not yet probed by us — the check runs on the deployed self-host instance and will be recorded here |
