...
| ChatGPT (web, mobile web, desktop) | Custom connector by URL, so the key travels as `?key=` | Yes: ChatGPT implements the MCP Apps standard (`ui://` resources, `text/html;profile=mcp-app`); widgets render on chatgpt.com, in a mobile browser and in the desktop app (owner's tests, 2026-09-02) | Yes, but it **sends** rather than prefills: desktop shows a send-or-cancel dialog; on the web with the connector set to "allow all" the message went out with no dialog | Yes: a widget-level `load` binding fired on first render, so `serverTools` is present and the frame's `tools/call` is proxied | Permission settings are still being explored; the connector badge read "CSP off" during these tests |
