...
- A template has no expressions. Data **selects** — `bind` emits a value as text, `map` picks an author-written literal, `prefix` composes an author-written prefix with a value, `format` presents a value through a closed number/currency/date vocabulary — and the author supplies every literal. Bindings only ever produce text and attribute strings, never markup.
