...
- **An MCP server.** Eight tools let any agent discover your catalog, read the authoring contract and render: `list_widgets`, `list_schemas`, `list_actions`, `list_themes`, `list_theme_tokens`, `get_authoring_guide`, `render_widget`, and `execute_action`, which widgets call for their own `http` actions.
