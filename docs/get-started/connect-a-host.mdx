...
ChatGPT implements the MCP Apps standard, so widgets mount inline there as in claude.ai; a basic test in ChatGPT Desktop rendered a widgentic widget (2026-09-02), and action behaviour is not yet probed. Its connectors take a URL only, so the key travels as a query parameter, the same shape as claude.ai's:
...
| ChatGPT | Inline (MCP Apps; basic test in ChatGPT Desktop) | `?key=` in the connector URL |
