...
ChatGPT's connectors take a URL only, so the key travels as a query parameter, the same shape as claude.ai's: `https://mcp.widgentic.dev/mcp?key=<api-key>`. Inline widgets there are **not probed**: widgentic serves the MCP Apps `ui://` template, not ChatGPT's Apps SDK `openai/outputTemplate` convention, so expect the text result (the HTML fragment plus the payload block) until a probe says otherwise. Where ChatGPT does take part today is the other direction — its desktop app's browser runs the authoring page's WebMCP tools, so an agent drafts into the designer you have open and you save; see [Co-author with a browser agent](/develop/co-author-with-an-agent).
...
| ChatGPT (connectors) | Text (not probed; MCP Apps template, not the Apps SDK convention) | `?key=` in the connector URL |
