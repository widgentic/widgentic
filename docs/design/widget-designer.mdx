...
The widget designer is where you write a custom widget. It runs in the app at [widgentic.dev](https://widgentic.dev) under **Widgets**, and the same component can be mounted in your own pages (see [Embed the designers](/develop/embed-the-designers)). It edits data, not code: the draft is the exact `CustomWidget` shape the server stores, every edit re-runs widgentic's own validators, and the preview is rendered through the real catalog, template and theming pipeline.
