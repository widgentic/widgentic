...
## Layout: what is public, what moves with the apps

`packages/core`, `packages/designer`, `packages/mcp` and `examples/*` are the
public monorepo; `apps/*`, `infra/` and the deployment sections of this file
(Production, The widgentic.dev app, the verification log entries that name
deploys) move with the apps to the private repository in Phase 2 of the split.
Package testing — the suites under `packages/*/src/**/__tests__`, the boundary
and export-snapshot checks under `tools/`, `npm run pack:check` — stays here.
...
**Currently deployed: `v58`** (v57 rebuilt from the npm-workspaces layout — `packages/core|designer|mcp`, `apps/*`, `examples/*` — no behavior change; the image runs on package sources through the root tsconfig `paths`; v57 = v56 + closing polish from live use: the action designer's Test call button spins while the host call runs, the Load action section shares the compact binding-editor chrome and completes from the widget's root data paths; v56 = v55 + actions-hardening — the deep-review closure: template tag denylist (`FORBIDDEN_TAG`), declared-args-only bindings with author-fixed query/headers winning, `net.BlockList` SSRF guard with embedded-IPv4 re-check, one 8 s request deadline through transport and body, bridge cycle counter / 30 s action timeout / keyboard parity on action anchors / load-after-initialize, strict envelope decoding and KEK versioning from the key id, load-aware store writes and raw-entry integrity scans, live-state designer editors, `POST /api/action-test` sharing the execution limiter, `--noUnusedLocals --noUnusedParameters` in typecheck; v55 = v54 + verification pass: execution limiter extracted to `src/mcp-server/rate-limit.ts`, no behavior change; v54 = designer round 3: action menu only on button/a, schema-copy label, busy states, top notification banner; v53 = designer round 2: attribute-bind path dropdowns, real selects in the mapping editors; v52 = designer fixes: output-map completions from both schemas with a type-mismatch flag, ref-resolved path dropdowns; v51 = actions inside `group` renders: descriptors carry `widget`/`at`, `execute_action` takes `at`/`item`, group loads; v50 = model-facing `Action notes`, fresh-listing tool description, actions in the authoring guide; v49 = widget actions: `execute_action`, the app template's action layer, the secrets KEK in `widgentickv` with both identities granted wrap/unwrap; v47 carried the temporary probe, v48 removed it) (both container apps share one image; check with `az containerapp list -g widgentic-rg --query "[].properties.template.containers[0].image"`).
...
- **Package split readiness — workspaces in place (2026-08-27, v58, gates + tarball smoke + deploy)** — the repository became an npm workspace without changing behavior: 14 modules moved (`git mv`) into `packages/core` (contract, adapters, mapper, catalog, theming, templates, actions, reactive, shared), `packages/designer` and `packages/mcp` (output convention, server, store, secrets); 74 files re-pointed by a codemod from `widgentic/<module>` and cross-module relative paths to `@widgentic/core`, `@widgentic/designer`, `@widgentic/mcp[/sdk|/store|/store/cosmos|/secrets|/secrets/keyvault]`. Gates: typecheck clean, 87 files / 900 tests (vitest projects per package + a `tools` project), the new boundary test (edge table, declared `exports` only, no `node:`/`Buffer`/`process` in core or designer, SDK and zod only from the `/sdk` assembly — its first run found one real crossing, the designer example reaching into the MCP example; fixed via the `@widgentic-examples/mcp-server/widgets` workspace package) and the export snapshots for all 16 entries. `npm run build` emits `dist` for the three packages plus the 132 KB single-file designer bundle; `npm run pack:check` (dist-only tarballs, publint strict, are-the-types-wrong esm-only) is green. Tarball smoke in a scratch Node 22 project: every entry imports with types (tsc), `@widgentic/mcp/sdk` fails with `ERR_MODULE_NOT_FOUND` until its optional peers are installed and exports `createWidgenticServer` afterwards, and `@widgentic/designer` mounts in a happy-dom-only context. Apps: `npm run mcp:http` (7 tools) and `npm run web` (bundle served) run on sources via tsx + `paths`. Deploy: ACR run `cj1u` from the workspace Dockerfile (`npm ci` over the workspace, no build step), `az deployment group create` Succeeded; `/healthz` 200, keyless catalog 7 tools, served `app.bundle.js` byte-identical to the local workspace build. Two things worth keeping: `npm pack --json -w` returns an object keyed by package name (not an array), and a boundary scanner must anchor `from "…"` to import/export statements or it will match prose in comments and strings.
- **Actions hardening — review closure (2026-08-26, v56, rollout check + served bytes)** — the 43-task `actions-hardening` change applied end to end; full suite 86 files / 881 tests green, typecheck clean with the new unused-symbol flags, `openspec validate --strict` valid. Rollout check: a one-off READ-ONLY script (CLI identity, Cosmos data-plane role) composed every production principal's stored widgets and actions with the new validators — 5 principals, 8 widgets, 1 shared action, 3 bindings, **zero** `FORBIDDEN_TAG`, undeclared-argument, output-grammar or compose hits, so nothing stored broke under the stricter rules. Deploy: `az acr build` run `cj1s` → `az deployment group create` Succeeded (41 s); both apps report image `widgentic-mcp:v56` (`widgentic-mcp--0000056`, `widgentic-web--0000047`, Running), `/healthz` 200. Served bytes: the public `app.bundle.js` carries the v56 client (`wgd-rec-pending` editor-local rows, `/api/action-test`, `busyControl`) and the sign-in copy names schemas/actions/secrets; the MCP template was verified from the same source build (`buildAppTemplate()` contains `ACTION_TIMEOUT_MS = 30000`, `tabindex`/`role` decoration, `pendingLoads`, the cycle guard and `data-wg-title` stash; `execute_action`'s wire schema derives its six property descriptions from `EXECUTE_ACTION_TOOL`) — the keyed `resources/read` was not exercised from this session (no production key at hand). Two finder mistakes worth keeping: `net.BlockList` treats IPv4 as mapped IPv6, so a `::ffff:0:0/96` rule blocks EVERY IPv4 address (the embedded-address re-check does that job instead); and a per-request `createExecutionLimiter` inside a request callback limits nothing — build it once per server. **Closing polish (v57, user findings on v56):** the Test call button only showed "…" in the output while the host call ran — it now carries the designer's `wgd-busy` spinner and `aria-busy`; the Load action section rendered with full-size controls unlike an element's binding editor in the tree — it now opts into the same compact chrome (`wgd-compact`), and its input-mapping paths complete from the widget's root data schema instead of flagging `place` as off-schema.
