...
| `widget-catalog` | `widgentic/catalog` | Built-ins (`card`, `table`, `tree`, `custom`, `group` — several widgets in one render with layout hints), registration API, pure `WidgetNode` render tree, `renderToHtml` + `mountNode`, image-aware card fields and table cells (avatars/thumbs/heroes via `hints.images`), descriptor data schemas incl. ReDoS-bounded `pattern` |
